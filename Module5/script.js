...
/**
* Сортировка, которая берет за начало указанную позицию и количетсво необходимое для сортировки
*/
// 
let array = [5, 9, 4, 7, 8, 0, 1, 2, 3];


function partSort(arr, start, length){
  let sum = start + length;
  let arrayCopy = arr;
  if(length){
    let arraySlice = arrayCopy.slice(start, sum).sort();
    // log(array);
    arrayCopy.splice(start, length, ...arraySlice);
    return arrayCopy;
  }
  let arraySlice = arrayCopy.slice(start).sort();
  arrayCopy.splice(start, arraySlice.length, ...arraySlice);
  return arrayCopy;
}

log(partSort(array, 3, 3));
